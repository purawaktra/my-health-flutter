...
class PartnerToUserPermissionHistoryScreen extends StatefulWidget {
  final AccessEntry partnerEntry;
  const PartnerToUserPermissionHistoryScreen(
      {Key? key, required this.partnerEntry})
      : super(key: key);

  @override
  _PartnerToUserPermissionHistoryScreenState createState() =>
      _PartnerToUserPermissionHistoryScreenState();
}

class _PartnerToUserPermissionHistoryScreenState
    extends State<PartnerToUserPermissionHistoryScreen> {
  ...
  Future<String> downloadAccessFile() async {
    ...
    File downloadToFile = File(
        '${_externalDocumentsDirectory!.path}/Akses Rekam Medis/${this.widget.partnerEntry.uid}');
    try {
      await storage
